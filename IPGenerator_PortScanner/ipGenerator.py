                    ...
                    for octet in lst1:
                        for octet2 in lst2:
                            for octet3 in range(int(i), int(j) + 1):
                                print(".".join([str(p) for p in fst[0:genindx]]) + ".{}.{}.{}".format(octet, octet2, octet3))
                    del lst1
                elif genindx == 2:
                    for octet in lst2:
                        for octet2 in range(int(i), int(j) + 1):
                            print(".".join([str(p) for p in fst[0:genindx]]) + ".{}.{}".format(octet, octet2))
                    del lst2
                else:
                    for octet in range(int(i), int(j) + 1):
                        print(".".join([str(p) for p in fst[0:genindx]]) + ".{}".format(octet))
            else:
                pass
    # print(" [+] Done")
